...
if __name__ == "__main__":
    ...
    dataset._data.y[:, args.target] = (dataset._data.y[:, args.target] - mean) / std
    print(f"Mean: {mean}, Std: {std}", flush=True)
    print(f"Normalized mean: {dataset._data.y[:, args.target].mean()}, normalized std: {dataset._data.y[:, args.target].std()}", flush=True)
    ...
    model_filename = os.path.join(args.save_dir, f'qm40_net_best_model_{args.target}.pth')
